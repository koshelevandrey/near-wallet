  ...
  const [dropdownVisible, setDropdownVisible] = useState(false)
   ...
   {!dropdownVisible ?
     ...
     <div className='walletsContainer'>
      ...
      {
        wallets.map(el => {
          return   el?.id !== wallet?.id ? <button onClick={() => {
            setDropdownVisible(!dropdownVisible)
            setWallet(el)
          }} key={el?.id} className='dropdownBtn interationBtn'>
      ...
      <button className='btnChooseNetwork' type='button' >Choose Network</button>
     </div>
   }
    <div className={`item titleContainer ${dropdownVisible ? 'visible' : ''}`}>
